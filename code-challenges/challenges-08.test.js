...
const oddValues = (arr) =>arr.filter(isOdd);
const isOdd=(n)=>n%2===1;
...
const filterStringsWithVowels = (arr) => arr.filter(isVowel);
const isVowel =n => (/[aouei]/gi.test(n))
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter( n => n.baseStat>minBaseStat);
...
const getStatName = (arr, minBaseStat) => (arr.filter( n => n.baseStat>minBaseStat)).map(obj=>obj.stat.name);
...
const getCharactersWithoutChildren = (arr) =>arr.filter(n=> ! n.children);
...
const evenOddNumericValues = (arr) => {
  return (arr.filter(n=> typeof n === "number" ) ).map(num=> num%2 === 0 ? 'even':'odd');
};
