...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(element => {
    ...
    template.children('h2').text(element.name);   
    template.children('h3').text(element.height);   
    template.children('p').text(element.eye_color);   
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(element => {
  let arrWithoutSpaces= element.split(' ');
  let foodName=arrWithoutSpaces.slice(2).join(' ');
...
const extractVowels = (str) => {
  ...
  vowelsRemoved=vowelsRemoved.split('').sort().join('');

  return [strAfterRemove,vowelsRemoved];
};
