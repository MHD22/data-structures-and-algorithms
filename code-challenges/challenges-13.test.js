...
const onlyOddChars = (str) => {
  ...
  for(let i = 0 ; i< str.length ; i++){
    res= i%2 ===1 ? res+ str.charAt(i) : res ;
  }
  return res;
...
const sortByDay = (arr) => {
  ...
  let saturday=[];
  let sunday=[];
  let weekEvents = [monday,tuesday,wednesday,thursday,friday,saturday,sunday];
  arr.forEach(str=>{
    daysOfWeek.forEach((day,ind)=>{
      if(str.includes(day)){
        weekEvents[ind].push(str);
      }
    })
...
const characterByIndex = (arr) => arr.map((str,ind)=>str[ind]);
