...
const createServer = () => {
  ...
  app.get('/aboutme',(req,res)=>{
    res.json({name:'mohammad',age:23,job:'software developer'});
  })
  app.get('/favoritefoods',(req,res)=>{
    res.json(['Mandi','Shawerma','Burger','fish','Rice']);
  })
  app.get('*',(req,res)=>{
...
const totalSum = (input) => {
  let singleArray = input.reduce((acc,val)=>{
    return acc.concat(val);
  },[]);
  let sum = singleArray.reduce((acc,val)=>acc+val,0);
  return sum; 
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let filteredArray = input.map(val=>{
   return val.filter(val2=>{
      if(typeof val2 === "number" && val2 % 5 ===0  ){
        return true;
      }
 ...
 let modifiedArray = filteredArray.map(val=>{
   return val.map(val=>{
...
let findMaleAndFemale = (data) => {
  let namesObj= data.filter(val=>{
      if(val.gender === 'male' || val.gender === 'female'){
        return true;
      }
  });
  let namesArray = namesObj.map(val=>{
    return val.name;
  });
  let namesString = namesArray.join(' and ');
  return namesString;
...
let findShortest = (data) => {
  let shortest = data.reduce((acc,val)=>{
    if(acc.length == 0 || val.name.length <= acc.length){
      return val.name;
    }
    else{
      return acc;
