...
const forEachTwoToThe = (arr) => {
  let outArray = [];
  arr.forEach(element => {
    outArray.push(Math.pow(2,element));
  });
  return outArray
};
...
const charCode = (arr) =>arr.map(num=> num.charCodeAt(0));
...
const evenOdd = (arr) =>arr.map(num=> num%2===0? 'even': num%2 ===1? 'odd':'N/A');
