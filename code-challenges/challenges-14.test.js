...
let biggerThanLuke = (arr) => {
  let lukeMass =77;
  let arr2 = arr.filter(val=> parseInt(val.mass)> lukeMass);
  let namesArray = arr2.map(val=>val.name);
  let result = namesArray.join(' - ');
  return result;
};
...
const detectTicTacToeWin = (board) => {
  ...
  let colsRowsDiagonalsArray = [row1,row2,row3,col1,col2,col3,diagonal1,diagonal2];

  let patternX=/X/g;
  let patternY=/O/g;

debugger;
  for (let i = 0; i < colsRowsDiagonalsArray.length; i++) {
    if( colsRowsDiagonalsArray[i].match(patternX) &&(colsRowsDiagonalsArray[i].match(patternX).length===3  )){
      return true;
    }
    if(colsRowsDiagonalsArray[i].match(patternY) &&colsRowsDiagonalsArray[i].match(patternY).length===3){
      return true;
    }
